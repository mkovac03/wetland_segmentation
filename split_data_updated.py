...
def generate_splits_and_weights(config):
    ...
    # Extract target EPSG from folder name
    epsg_zone = None
    epsg_match = re.search(r"epsg(326\d{2})", input_dir)
    if epsg_match:
        epsg_zone = epsg_match.group(1)
        print(f"[INFO] Restricting to tiles with EPSG:{epsg_zone}")
    ...
    for base in tqdm(file_list):
        if epsg_zone and epsg_zone not in base:
            continue
    ...
    tiles = list(zip(X_full, E_full, Y_full))
    tiles = sorted(tiles, key=lambda t: -t[1])  # sort by entropy
    if entropy_cut:
        top_n = int(len(tiles) * entropy_cut / 100)
        tiles = tiles[:top_n]
    if max_tiles:
        tiles = tiles[:max_tiles]

    X = np.array([t[0] for t in tiles])
    Y = np.stack([t[2] for t in tiles])
    print(f"[INFO] Total selected tiles: {len(X)} after filtering")
