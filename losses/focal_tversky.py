...
class CombinedFocalTverskyLoss(nn.Module):
    def __init__(self, num_classes, focal_alpha=0.25, focal_gamma=1.0, tversky_alpha=0.5, tversky_beta=0.7, boundary_weight=0.2, ignore_index=255):
        super().__init__()
    ...
    def forward(self, inputs, targets):
        ...
        boundary_mask = self._compute_boundary_mask(targets).to(inputs.device)
        ...
        one_hot = torch.nn.functional.one_hot(targets.clamp(max=self.num_classes - 1), num_classes=self.num_classes)
        one_hot = one_hot.permute(0, 3, 1, 2).float().to(inputs.device)
        ...
        return 0.5 * focal_loss + 0.5 * tversky_loss
